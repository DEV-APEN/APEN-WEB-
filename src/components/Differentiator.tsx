...
export default function Differentiator() {
  ...
  return (
    <section className="relative min-h-screen bg-[#0B2341] py-24 flex flex-col justify-center overflow-hidden">
      
      {/* KINETIC BACKGROUND: SCANNING GRID */}
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10 w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          ...
          >
            <div>
              <div className="flex items-center gap-4 mb-8">
                ...
                <span className="text-[#005BB5] font-black text-[12px] tracking-[0.5em] uppercase border-l border-white/10 pl-4">Operating Interface 01_Diff</span>
              </div>
              
              <h2 className="text-4xl md:text-7xl font-black text-white tracking-tighter uppercase leading-[0.85] mb-8">
                Un paso <br />
                <span className="text-[#005BB5]">más allá</span> del <br />
                contratista.
              </h2>

              <p className="text-blue-100/80 text-lg md:text-xl font-bold max-w-lg mb-8 leading-relaxed">
              ...
              <div className="flex flex-wrap gap-4 pt-8 border-t border-white/5">
                {['Socio Estratégico', 'Integración 360°', 'Trazabilidad Técnica'].map((tag) => (
                   <span key={tag} className="text-[10px] font-black uppercase tracking-widest text-[#005BB5]/70 flex items-center gap-2">
            ...
            <div className="flex items-center gap-4">
               <button 
                 ...
                 className={`group relative overflow-hidden px-10 py-4 rounded-sm transition-all duration-500 border ${view === 'tradicional' ? 'bg-red-600 border-red-600 text-white shadow-[0_0_30px_rgba(220,38,38,0.4)]' : 'bg-transparent border-white/10 text-white/40 hover:text-white'}`}
               >
                 <span className="relative z-10 text-[10px] font-black uppercase tracking-widest">Modelo Tradicional</span>
                 {view !== 'tradicional' && <div className="absolute inset-0 bg-red-600/10 -translate-x-full transition-transform group-hover:translate-x-0 group-hover:duration-300"></div>}
               </button>

               <button 
                 onClick={() => setView('apen')}
                 className={`group relative overflow-hidden px-10 py-4 rounded-sm transition-all duration-500 border ${view === 'apen' ? 'bg-[#005BB5] border-[#005BB5] text-white shadow-[0_0_30px_rgba(0,91,181,0.4)]' : 'bg-transparent border-white/10 text-white/40 hover:text-white'}`}
               >
                 <span className="relative z-10 text-[10px] font-black uppercase tracking-widest">Modelo APEN</span>
                 {view !== 'apen' && <div className="absolute inset-0 bg-[#005BB5]/10 -translate-x-full transition-transform group-hover:translate-x-0 group-hover:duration-300"></div>}
               </button>
            </div>
          </motion.div>
          ...
          <div className="relative h-[500px] md:h-[600px] flex items-center justify-center [perspective:1000px]">
            ...
            {/* CENTRAL REACTOR CORE (Rotación Forzada a 0) */}
            <motion.div 
               animate={{ 
                 rotate: 0, 
                 scale: view === 'apen' ? [1, 1.02, 1] : 0.9, 
               ...
               className="relative w-80 h-80 rounded-full border border-[#005BB5]/30 flex items-center justify-center group"
            >
               <div className="absolute inset-0 border-2 border-dashed border-[#005BB5]/10 rounded-full"></div>
               <div className="absolute inset-0 bg-gradient-to-b from-[#005BB5]/10 to-transparent rounded-full opacity-10"></div>

               {/* Inner Nucleus */}
               <div className="relative z-20 w-32 h-32 md:w-44 md:h-44 bg-[#061528] rounded-full border border-white/10 shadow-[0_0_100px_rgba(0,91,181,0.3)] flex flex-col items-center justify-center overflow-hidden">
            ...
            {/* SATELLITE MODULES (Kinetic Interaction) */}
